...
public final class SqliteConnectionManager extends SqliteConnectionFactory {
    ...
    public void init() {
        ...
        try {
            SqlHelper.execute(getConnection(), String.format(SqliteStorage.LOG_CHAT_CREATION_STATEMENT, STORAGE_SQLITE_TABLES_CHAT_LOGS.get(plugin.getSpaceChatConfig().getAdapter())));
            SqlHelper.execute(getConnection(), String.format(SqliteStorage.USERS_CREATION_STATEMENT, STORAGE_SQLITE_TABLES_USERS.get(plugin.getSpaceChatConfig().getAdapter())));
            SqlHelper.execute(getConnection(), String.format(SqliteStorage.USERS_SUBSCRIBED_CHANNELS_CREATION_STATEMENT, STORAGE_SQLITE_TABLES_SUBSCRIBED_CHANNELS.get(plugin.getSpaceChatConfig().getAdapter())));

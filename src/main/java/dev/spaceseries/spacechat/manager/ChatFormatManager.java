...
public class ChatFormatManager extends FormatManager {
    ...
    public void send(AsyncPlayerChatEvent event, String message) {
        ...
        SpaceChat.getInstance().getDynamicConnectionManager().getRedisSupervisor().publishChatMessage(new RedisChatPacket(player.getUniqueId(), player.getName(), REDIS_SERVER_IDENTIFIER.get(Config.get()), REDIS_SERVER_DISPLAYNAME.get(Config.get()), components));
